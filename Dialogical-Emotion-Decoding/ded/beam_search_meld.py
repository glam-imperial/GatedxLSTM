...
class BeamSearch(object):
  ...
  def decode(self, dialog):
    ...
    beam_set = [BeamState([], [], 0, [0] * self.num_state) for _ in range(self.beam_size)]
  ...
  def _update_beam(self, beam_state, utt_id, state):
    ...
    loss = 0
    # speaker = utt_id[-4]
    speaker = utt_id.split('_')[-1]  # ✅ 提取 Speaker 名字，如 "Mark", "Rachel"
    # Convert the state into one-hot vector
    label = np.zeros(self.num_state)
    label[state] = 1

    # Get original ce loss
    logit = np.reshape(self.all_logits[utt_id], [7,])
    loss = utils_meld.cross_entropy(label, logit)
    ...
    if state in np.unique(beam_state.emo_sequence) and speaker in np.unique(beam_state.spk_sequence): 
      # Find last state
      # last_idx = utils_meld.find_last_idx(beam_state.spk_sequence, speaker)
      # last_state = beam_state.emo_sequence[last_idx]

      last_idx = utils_meld.find_last_idx(beam_state.spk_sequence, speaker)

      if last_idx is None:
          print(f"Warning: Speaker {speaker} not found in spk_sequence {beam_state.spk_sequence}")
          last_state = None  # 没找到上一个状态，跳过
      else:
          last_state = beam_state.emo_sequence[last_idx]
